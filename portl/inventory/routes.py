...
from portl.extensions import db
from portl.functions import factory, fetch, fetch_all, get, get_one, objectify, post
from portl.inventory import bp
from portl.inventory.forms import (
    AddDevice,
    AddLink,
    AddPoolForm,
...
from portl.inventory.functions import get_pools_devices, object_export, object_import
...
def import_export() -> dict:
    return dict(
        ...
        librportl_form=LibrportlForm(request.form),
...
def connection(device_id: int) -> dict:
    ...
    return {
        ...
        "server_addr": app.config["portl_SERVER_ADDR"],
    }
...
@post(bp, "/query_librportl", "Edit")
def query_librportl() -> bool:
    devices = http_get(
        f'{request.form["librportl_address"]}/api/v0/devices',
        headers={"X-Auth-Token": request.form["librportl_token"]},
    ).json()["devices"]
    for device in devices:
        factory(
            ...
            **{
                ...
                "subtype": request.form["librportl_type"],
                "longitude": 0.0,
                "latitude": 0.0,
            },
        )
