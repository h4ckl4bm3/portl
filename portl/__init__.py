...
from portl.admin.functions import configure_instance_id
from portl.admin.models import User
from portl.classes import classes, service_classes
from portl.cli import configure_cli
from portl.config import Config
from portl.default import create_default
from portl.examples import create_examples
from portl.extensions import (
    controller,
    db,
    login_manager,
    mail_client,
...
from portl.functions import fetch
from portl.logs.models import SyslogServer
from portl.properties import (
    boolean_properties,
    cls_to_properties,
    device_subtypes,
    google_earth_styles,
...
from portl.rest import configure_rest_api
...
def register_blueprints(app: Flask) -> None:
    ...
    for blueprint in blueprints:
        module = import_module(f"portl.{blueprint}")
...
def configure_logs(app: Flask) -> None:
    basicConfig(
        level=getattr(import_module("logging"), app.config["PORTL_LOG_LEVEL"]),
        format="%(asctime)s %(levelname)-8s %(message)s",
        datefmt="%m-%d-%Y %H:%M:%S",
        handlers=[
            RotatingFileHandler(
                app.path / "logs" / "app_logs" / "portl.log",
                maxBytes=20_000_000,
                backupCount=10,
            ),
            StreamHandler(),
...
def configure_google_earth(path: Path) -> None:
    for subtype in device_subtypes:
        ...
        path_icon = f"{path}/portl/views/static/images/2D/{subtype}.gif"
...
def configure_services(path: Path) -> None:
    path_services = [path / "portl" / "automation" / "services"]
