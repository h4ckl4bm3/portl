...
class Config(object):
    ...
    # portl
    PORTL_SERVER_ADDR = environ.get("PORTL_SERVER_ADDR")
    ...
    # Examples
    CREATE_EXAMPLES = int(environ.get("CREATE_EXAMPLES", False))
    ...
    MAIL_SENDER = environ.get("MAIL_SENDER", "portl@portl.fr")
    ...
    # Logging
    PORTL_LOG_LEVEL = environ.get("PORTL_LOG_LEVEL", "DEBUG").upper()

    # Cluster
    # In production, for scalability and high-availability purposes, it is
    # recommended to deploy not one, but multiple instances of portl.
...
class DebugConfig(Config):
    DEBUG = True
    SECRET_KEY = environ.get("PORTL_SECRET_KEY", "get-a-real-key")

    # Database
    SQLALCHEMY_DATABASE_URI = environ.get(
        "PORTL_DATABASE_URL", "sqlite:///database.db?check_same_thread=False"
    )
...
class ProductionConfig(Config):
    ...
    SECRET_KEY = environ.get("PORTL_SECRET_KEY")
    ...
    SQLALCHEMY_DATABASE_URI = environ.get(
        ...
        "postgresql://{}:{}@{}:{}/{}".format(
            ...
            environ.get("POSTGRES_PASSWORD", "portl"),
    ...
    # stored in a vault.
    # There MUST be a Vault to use portl in production mode securely.
