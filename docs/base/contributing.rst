...
Contributions are welcome. If you want to contribute, you should join the #portl channel in the networktocode slack (http://networktocode.herokuapp.com/).

For developers
--------------

portl uses flake8 to make sure that the python code is PEP8-compliant, eslint to make sure the javascript code is compliant with google standards for javascript, and pytest for the test suite.
...
Both Black and Mypy are part of the portl CI/CD process. Black is a code formatting enforcement tool; see (https://github.com/ambv/black). And Mypy is a adds static type hinting to Python for the whole project; see (http://mypy-lang.org/).
...
portl has a CLI interface available for developers, with the following operations:
...
However, because services are loaded programatically by portl, they are not being monitored by Flask and a change to a service will not trigger the application reload.
